...
function validateBody(body: Record<string, unknown>): string | null {
  const { type, subject, message } = body as Record<string, string>;
  if (type === "individual") {
    const { name, email, role, university } = body as Record<string, string>;
    if (missing(name, email, role, subject, message) || !EMAIL_RE.test(email)) return "Missing or invalid fields";
    if (typeof name === "string" && name.length > 80) return "Name too long";
    if (typeof email === "string" && email.length > 254) return "Email too long";
    if (typeof university === "string" && university.length > 120) return "University name too long";
    if (typeof subject === "string" && subject.length > 120) return "Subject too long";
    if (typeof message === "string" && message.length > 2000) return "Message too long";
    if (role === "Student" && missing(university)) return "University required for students";
  } else if (type === "team") {
    const { repName, email, org, role } = body as Record<string, string>;
    if (missing(repName, email, org, role, subject, message) || !EMAIL_RE.test(email)) return "Missing or invalid fields";
 ...
 * Runs captcha + validation + rate limiting, stores the message server-side, and
 * emails a code. The actual message to the team is NOT sent until the code is
 * confirmed. Rate limiting lives here so verification emails can't be abused.
 */
async function handleRequest(req: NextRequest, body: Record<string, unknown>) {
  ...
  try {
    const code = await createVerification(submitterEmail, "contact", payload);
    await Promise.all([
      sendVerificationEmail(resend, submitterEmail, code, "contact"),
      sql`
    ...
    console.error("Contact verification request error:", err);
    return NextResponse.json({ success: false }, { status: 500 });
  }
}

/**
 * STEP 2 — confirm the code, then deliver the message.
 * The message is only emailed to the team once the code matches server state.
 */
async function handleVerify(body: Record<string, unknown>) {
  ...
  const result = await verifyCode(email, "contact", code);
  if (!result.ok) {
  ...
  const { type, subject, message } = payload as Record<string, string>;

  let emailSubject: string;
  let html: string;

  if (type === "team") {
    const { repName, email: rawEmail, org, role } = payload as Record<string, string>;
    ...
    const { name, email: rawEmail, role, university } = payload as Record<string, string>;
  ...
  const userConfirmHtml = `
    <div style="font-family: monospace; background: #000; color: #e2e8f0; padding: 32px; border-radius: 12px; border: 1px solid rgba(37,99,235,0.3);">
      ...
      <div style="border: 1px solid rgba(255,255,255,0.08); border-radius: 8px; padding: 16px; margin-bottom: 24px;">
        ...
        <p style="color: #94a3b8; line-height: 1.6; margin: 0; white-space: pre-wrap;">${esc(message)}</p>
      </div>
      <p style="color: #64748b; font-size: 12px; border-top: 1px solid rgba(255,255,255,0.08); padding-top: 16px; line-height: 1.6;">
        If this is your first time receiving an email from <strong style="color: #94a3b8;">${SENDER_EMAIL}</strong>, please check your spam folder and unmark us as spam if applicable — we&apos;d hate for our reply to end up there.
      </p>
    </div>
  `;
